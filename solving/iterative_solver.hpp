...
namespace Picross
{
    class IterativeSolver : public Solver
    {
        ...
        private:
            ...
            static std::vector<cell_t> arrayTrivialCheck(std::vector<cell_t> array, std::vector<int> hints);
            // Cross cells that are out of bounds for any checked block according to given hints.
            static std::vector<cell_t> arrayTrivialCross(std::vector<cell_t> array, std::vector<int> hints);
            // Cross cells that are in spaces where no checked blocks fit according to given hints.
            static std::vector<cell_t> arrayForceCross(std::vector<cell_t> array, std::vector<int> hints);
            // Check additional cells in blocks that are close enough to a border or a crossed cell according to given hints.
            static std::vector<cell_t> arrayGlue(std::vector<cell_t> array, std::vector<int> hints);
